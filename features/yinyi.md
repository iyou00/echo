...
**这是 Echo 与播放器最不一样的地方**。播放器只服务"现在",风信服务"过去和回望"。
...
| US-2 | 普通用户 | 翻开风信页就看到今天 / 最新这篇 | 不用去找 |
...
| US-8 | 想保存的用户 | 把某一篇风信导成图片或卡片分享 | (v0.3+ 候选) |

## 3 · 输入(Inputs)· 写一篇风信需要什么
...
interface YinyiContext {
  ...
  recentYinyi: Array<{             // 最近 7 天的风信,避免重复观察
    date: string;
    content: string;
  }>;
...
[加载 YinyiContext] 
     │
     ├─ 素材足够(对话 ≥ 3 条 OR 听歌 ≥ 5 首)─→ 生成正常风信
     │
     ├─ 素材稀少 ────────────────────────────→ 生成"短风信"(50-100 字)
     │
...
| 场景 | 处理 |
|---|---|
| 用户今天完全没打开 app | 不生成正式风信,只在 `yinyi` 表存 `status='absent', content='今天没见到你'`,翻页时能看到一行小字"那天我们没见" |
...
| LLM 生成失败 | 不写空风信,留 `status='generation_failed'`,第二天可手动重试 |

### 5.3 用户手动触发

设置页 / 风信页有"重新生成今日风信"按钮(藏一点,不主推)。重新生成会覆盖。

## 6 · prompt 工程 · 如何让它别像 AI 写的

这是风信能不能"有人味"的关键。问题在于 LLM 默认会写得**太工整、太对仗、太鸡汤**。
我们用三层手段对抗这个倾向:
...
不及格就重写一次。代价是双倍 token,但风信是低频任务(一天一次),可以接受。

### 6.4 温度控制

`temperature = 0.85`(对话默认是 0.7)。风信需要更高的"随机性",
不然每天都会写得太像。
...
- **没生成的日子(absent / failed)** 翻到时显示:
  ...
  - failed:中央一句"那天的风信我没写好,要不试试重新生成?"+ 重生成按钮
...
- 下一篇风信生成时,prompt 注入"昨天用户对你的某一观察做过纠正"
...
// 主 → 渲染(广播)
'yinyi:generated'  → { date }     // 新风信生成完通知,可弹小提示
```
...
| 用户连续 5 天没用,第 6 天回来 | 第 6 天的风信中提及"好久没见" |
| 用户在 22:00 正好打字 | 推迟到空闲时再触发 |
| 用户主动重生成今天的 | 覆盖之前的 |
| 翻到一个 absent 的日子 | 显示"那天我们没见" |
| 设置里关掉自动生成 | scheduler 不触发(v0.2 加这个开关) |
| LLM 失败 | 不留空风信,标 failed |
| 连续 5 天风信开头都是"今天" | 多样性检查介入,要求重写 |
