...
70-100 字纯文本(单段,不分段)。**LLM 不返回结构化数据**,scenario 不像音忆那么大,简单。
...
| 与 `recommendation` | scenario 文案里**可以**提到一首歌("我推 X 给你"),但**不直接触发播放**——只口播。用户回到主对话页可以追问"放刚才那首" |
| 与 `taste-profile` | 不读取 / 不修改 |
| 与 `yinyi` | scenario 不进音忆素材(它不是用户行为) |
