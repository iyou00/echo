...
截至 2026-08-14，D2.0 至 D2.5 已在 `echo-new` 完成：

| 阶段 | 状态 | 结果 |
| --- | --- | --- |
| D2.0 | 完成 | 建立迁移基线、文件所有权和回归标准 |
| D2.1 | 完成 | 新外壳、三档固定尺寸、TopBar、抽屉和 WindowField |
| D2.2 | 完成 | 聊天、播放、一起听、场景、队列、品味、风信和设置迁移 |
| D2.3 | 完成 | 受控边界码、恢复动作、离线与失败状态 |
| D2.4 | 完成 | 音乐前奏、动态文案、启动恢复和繁忙退出 |
| D2.5 | 完成 | 全量测试、Electron 真窗口验收、NSIS 安装/更新/卸载验证 |

最终验证结果：全量测试通过；1152×720、1280×800、1440×900 三档截图通过；首次启动有声/静音、正常/减少动态、媒体会话、繁忙退出、离线和启动失败恢复共 12 张 Electron 截图通过；NSIS 安装包在隔离目录完成干净安装、打包版首次启动、覆盖更新和完整卸载，更新及默认卸载均保留用户数据。
...
实施结果：通过。`npm run release:verify` 是发布质量门，依次执行 lint、全量测试、生产构建、Electron 截图验收、NSIS 打包和安装器冒烟测试。当前更新策略为新版 Setup 覆盖安装；在线自动更新需在后续接入可信发布源后单独实现，不在本阶段伪装支持。
...
4. 目标状态 Electron `capturePage` 截图及像素非空检查
...
- [x] 新建 theme tokens，并完成 D2 样式迁移。
- [x] 新建 `EchoShell`，承接现有页面与业务契约。
- [x] 实现三档固定窗口 IPC 和设置入口。
- [x] 实现 `WindowField` 的空闲、聊天、播放及欢迎状态。
- [x] 截图评审通过并完成 D2.2 至 D2.5。
