...
import type { CareFrequency, CarePingScheduleItem, SchedulerCatchupResult } from '../../types/ipc'
...
export function rescheduleYinyi(): void {
  ...
  yinyiTask = cron.schedule(`${minute} ${hour} * * *`, async () => {
    try {
      ...
      insertScheduledJob('yinyi_daily', entry.date, status, status === 'completed' ? '音忆已生成。' : '音忆生成失败。', entry.meta?.error)
      recordHealth('scheduler', status === 'completed' ? 'ok' : 'degraded', status === 'completed' ? '定时音忆已生成。' : '定时音忆生成失败。', entry.meta?.error)
      broadcastYinyiGenerated(entry.date, entry.meta?.status ?? 'ok')
    } catch (error) {
      const message = error instanceof Error ? error.message : '音忆定时任务失败'
      insertScheduledJob('yinyi_daily', todayIso(), 'failed', '音忆生成失败。', message)
  ...
  const records = activeWindows
    .map((window) => {
      ...
      if (!plannedAt) {
        ...
        const skipped = upsertCarePingPlan(date, window.key, window.label, fallback)
        updateCarePingPlanStatus(skipped.id, 'skipped', '今天这个提醒时段已经过去。')
...
export async function runStartupCatchup(): Promise<SchedulerCatchupResult> {
  const date = scheduledDateForNow()
  const existing = getByDate(date)
  if (existing) {
    insertScheduledJob('yinyi_daily', date, 'skipped', '这一天已经有音忆了。')
    recordHealth('scheduler', 'ok', '定时任务已恢复。')
    return { ok: true, job: 'yinyi_daily', date, status: 'skipped', message: '这一天已经有音忆了。' }
  }
  ...
  try {
    ...
    const status = entry.meta?.status === 'failed' ? 'failed' : 'completed'
    const message = status === 'completed' ? '已补写最近缺失的音忆。' : '补写音忆失败，已留下可重生成占位。'
    insertScheduledJob('yinyi_daily', date, status, message, entry.meta?.error)
    recordHealth('scheduler', status === 'completed' ? 'ok' : 'degraded', status === 'completed' ? '定时任务已恢复。' : '补写音忆失败。', entry.meta?.error)
    broadcastYinyiGenerated(entry.date, entry.meta?.status ?? 'ok')
