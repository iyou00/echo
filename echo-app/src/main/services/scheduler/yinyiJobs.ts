...
export function runYinyiDailyTask(date: string): Promise<SchedulerCatchupResult> {
  return runSchedulerResultTask({
    ...
    try {
      ...
      const message = status === 'completed' ? '风信已生成。' : '风信生成失败。'
      ...
      return { ok: status === 'completed', job: 'yinyi_daily', date: entry.date, status, message }
    } catch (error) {
...
async function runYinyiCatchupJob(signal?: AbortSignal): Promise<SchedulerCatchupResult> {
  ...
  if (existing) {
    ...
    recordSchedulerHealth('yinyi', 'ok', '已恢复。')
  ...
  try {
    ...
    recordSchedulerHealth('yinyi', status === 'completed' ? 'ok' : 'degraded', status === 'completed' ? '已恢复。' : '补写失败。', entry.meta?.error)
    broadcastYinyiGenerated(entry.date, entry.meta?.status ?? 'ok')
