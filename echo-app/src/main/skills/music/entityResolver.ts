...
const ACTION_PREFIX_PATTERN = /^(?:我)?(?:再来|再给我|再推荐|继续来|接着来|想要听|我要听|我想听|想听|要听|听听看|听一下|播放|放一下|放一首|放首|放点|推荐|推|给我|帮我|找一首|找首|来一首|来点|点播|整一首|整首|整点|安排一首|安排首|安排点|搞一首|搞首|搞点|弄一首|弄首|弄点|安排|整|搞|弄)\s*/i
...
const IMPLAUSIBLE_ARTIST_TERMS = /是|最|很|挺|特别|温暖|相遇|拥抱|听|想|要|播放|放|推荐|适合|值得|天气|心情|感觉|类似|相似|像|好听|耐听|顺耳|入耳|对味|舒缓|缓和|轻柔|安静|放松|激昂|热血|澎湃|带感|节奏|国外|外国|欧美|英文|粤语|华语|日语|韩语|法语|德语|西班牙语|俄语|泰语|葡萄牙语|意大利语|今天|明天|昨天|现在|最近|歌单|我们|你们|别的|其他|新歌|老歌|重金属|朋克|古典|纯音乐|随便|再来|继续|接着|还有|还要|另外|关于|有关|能把|能让|把心情|让心情|让情绪|这口气|散掉|散去|发泄|宣泄|出气|解压|堵得慌|心里堵|憋得慌|闷得慌|好起来|心情调|情绪调/
...
export function resolveMusicEntitiesFromText(text: string): MusicEntityResolution {
  ...
  if (!artistQuery && !(seedTitle && /(?:类似|像|相似|相近|差不多|同款|同类型|同风格)/i.test(trimmed))) {
    const artistPatterns = [
      // 「你觉得X有什么好听的」「X有什么好听（的歌）」——征询意见式点歌，X 是话题主语即歌手。
      // 限定词（好听/值得听…）必须有，否则「有什么歌」这类泛问会把前文误捕成歌手。
      /(?:你觉得|你认为|你说|听说|觉得|感觉)?\s*([A-Za-z0-9 .&'’\-\u4e00-\u9fa5]{1,24}?)有什么(?:好听|不错|值得听|经典|推荐)(?:的)?(?:歌|歌曲|音乐|作品)?/i,
      /(?:想听|想要听|听听|听一下|播放|放|来点|推荐|推|给我|帮我|找)\s*([A-Za-z0-9 .&'’\-\u4e00-\u9fa5]{1,32}?)(?:的)?(?:最新|新出(?:的)?|最近(?:发布|发行|上线|出)(?:的)?)\s*(?:(?:\d{1,2}|[一二两三四五六七八九十两几])\s*首)?(?:歌|歌曲|音乐|作品)/i,
      /([A-Za-z0-9 .&'’\-\u4e00-\u9fa5]{1,32}?)(?:的)(?:最新|新出(?:的)?|最近(?:发布|发行|上线|出)(?:的)?)\s*(?:(?:\d{1,2}|[一二两三四五六七八九十两几])\s*首)?(?:歌|歌曲|音乐|作品)/i,
      /(?:想听|想要听|要听|我要听|我想听|听|播放|放一首|放首|放|来一首|来点|推荐|推|给我|帮我|找一首|找首|整点|安排点|搞点|弄点)\s*(?:一首|几首|[一二两三四五六七八九十两几]\s*首|\d{1,2}\s*首)?\s*([A-Za-z0-9 .&'’\-\u4e00-\u9fa5]{1,32}?)(?:的)?(?:热门|热度(?:高|最高)|最火|人气(?:高|最高)|经典|代表作)(?:的)?(?:歌|歌曲|音乐|作品)?/i,
      ...
      /(?:想听|想要听|要听|我要听|我想听|听|播放|放一首|放首|放|来一首|来点|推荐|推|给我|帮我|找一首|找首|整点|安排点|搞点|弄点|整一首|安排一首|搞一首|弄一首|整首|安排首|搞首|弄首)\s*(?:一首|几首|[一二两三四五六七八九十两几]\s*首|\d{1,2}\s*首)?\s*([A-Za-z0-9 .&'’\-\u4e00-\u9fa5]{1,32}?)(?:的)?(?:热门|热度(?:高|最高)|最火|人气(?:高|最高)|经典|代表作|流行|出名|有名|火一点|火的|好听)(?:的)?(?:歌|歌曲|音乐|作品)?/i,
      /(?:整|安排|搞|弄)\s*(?:一首|首)\s*([A-Za-z0-9 .&'’\-\u4e00-\u9fa5]{1,32})(?:吧|呀|啊)?$/i,
      // 裸歌手收尾：「随便来点周杰伦」「听陈奕迅」——动词+名字+句尾。
      // 动词只取"听/来点"类（指向歌手）；「放/播 X」指向具体歌名，交给 titleOnly 路径。
      // 「一点/一些」后缀是描述性请求（来点暖一点的），整体排除；歌手名至少 2 字。
      // 依赖 normalize→描述词过滤→不可信词表三道闸，命中错词时由网易云验证兜底（verify 失败→追问而非乱放）。
      /(?:来点|听听|听|点播|找点)\s*(?![\s\S]*(?:一点|一些))([A-Za-z0-9 .&'’\-\u4e00-\u9fa5]{2,32}?)(?:的)?(?:吧|呀|啊|呗|啦|咯)?$/i,
      // 所有格收尾：「随便来一首苏星婕的」「放首周杰伦的」——动词+量词+名字+的。
      // 尾「的」是所有格标记，指向歌手；与「放一首晴天」（无「的」→歌名）天然区分。
      // 量词必选：「来点别的」这类无量词短语不能进此模式。
      /(?:来|放|找|推|要|整|安排|搞|弄)\s*(?:一首|几首|\d+\s*首|首)\s*([A-Za-z0-9 .&'’\-\u4e00-\u9fa5]{2,32})的$/i,
