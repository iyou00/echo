...
async function captureFirstRun(target: BrowserWindow): Promise<CaptureMetric[]> {
  ...
  await click(target, '.d2-nav-button[aria-label="设置"]')
  await waitForSelector(target, '.shell-page[data-page="settings"] .settings-page')
  await wait(360)
  metrics.push(await capture(target, 'settings-overview.png'))
  await click(target, '[data-testid="settings-rail-llm"]')
  await waitForSelector(target, '.d2-settings-form.detail-llm')
  await wait(160)
  metrics.push(await capture(target, 'settings-ai-model.png'))
  await click(target, '[data-testid="settings-rail-music"]')
  await waitForSelector(target, '.d2-settings-form.detail-music')
  await wait(160)
  metrics.push(await capture(target, 'settings-import.png'))
  await click(target, '[data-testid="settings-rail-tasks"]')
  await waitForSelector(target, '[data-testid="settings-tasks-view"]')
  await wait(120)
  metrics.push(await capture(target, 'settings-tasks.png'))
  await click(target, '[data-testid="settings-rail-yinyi"]')
  ...
  await click(target, '[data-testid="settings-rail-music"]')
  await wait(120)
  await click(target, '.d2-nav-button[aria-label="品味"]')
  await waitForSelector(target, '.shell-page[data-page="profile"] .d2-profile')
  await wait(360)
  metrics.push(await capture(target, 'profile-page.png'))
  await click(target, '.d2-brand')
  await waitForSelector(target, '.chat-page')
  await click(target, '[aria-label="打开队列"]')
  await waitForSelector(target, '.shell-page[data-page="queue"] .d2-queue')
  await wait(360)
  metrics.push(await capture(target, 'queue-page.png'))
  await click(target, '.d2-brand')
  await waitForSelector(target, '.chat-page')
  await click(target, '.d2-nav-button[aria-label="设置"]')
  await waitForSelector(target, '.shell-page[data-page="settings"] .settings-page')
  await wait(200)
  await click(target, '[data-testid="settings-rail-about"]')
  await waitForSelector(target, '.shell-page[data-page="about"] .d2-about')
  await wait(300)
  metrics.push(await capture(target, 'about-page.png'))
  await click(target, '.d2-brand')
  await waitForSelector(target, '.chat-page')
  await click(target, '.d2-nav-button[aria-label="风信"]')
  await waitForSelector(target, '.d2-yinyi')
  await wait(400)
  metrics.push(await capture(target, 'yinyi-empty.png'))
  await click(target, '.d2-brand')
  await waitForSelector(target, '.chat-page')
  await click(target, '.d2-nav-button[aria-label="设置"]')
  await waitForSelector(target, '.shell-page[data-page="settings"] .settings-page')
  await click(target, '[data-testid="settings-rail-learned"]')
  await waitForSelector(target, '.d2-settings-form.detail-learned .target-learned')
  await wait(300)
  ...
  metrics.push(await capture(target, 'listening-stage.png'))
  await click(target, '.d2-nav-button[aria-label="设置"]')
  await waitForSelector(target, '.shell-page[data-page="settings"] .settings-page')
  await wait(360)
  metrics.push(await capture(target, 'listening-settings-page.png'))
  await click(target, '.d2-brand')
  await waitForSelector(target, '.field-listening')
  await waitForSelector(target, '.field-listening')
  await click(target, '.d2-now-return')
  await waitForMissing(target, '.d2-now-return')
  await waitForExpression(target, `document.querySelector('.global-player audio')?.paused === false`)
  await wait(420)
  metrics.push(await capture(target, 'listening-compact.png'))
  await click(target, '.d2-sound-art')
  await waitForSelector(target, '.field-listening')
  await waitForSelector(target, '.d2-now-return')
  await click(target, '.d2-sound-art')
