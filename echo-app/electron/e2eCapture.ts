...
async function captureFirstRun(target: BrowserWindow): Promise<CaptureMetric[]> {
  ...
  await click(target, '[data-testid="settings-crumb-connections"]')
  await waitForSelector(target, '[data-testid="settings-connections-view"]')
  await click(target, '[data-testid="settings-music-edit"]')
  await waitForSelector(target, '.d2-settings-form.detail-music')
  await wait(160)
  metrics.push(await capture(target, 'settings-import.png'))
  await click(target, '[data-testid="settings-crumb-connections"]')
  await waitForSelector(target, '[data-testid="settings-connections-view"]')
  await click(target, '[data-testid="settings-crumb-overview"]')
  ...
  await click(target, '.d2-icon-button[aria-label="设置"]')
  await waitForSelector(target, '.d2-drawer-layer.open .settings-page')
  await click(target, '[data-testid="settings-learned-entry"]')
  ...
  await waitForSelector(target, '.d2-drawer-layer.open .settings-page')
  await wait(360)
  metrics.push(await capture(target, 'listening-settings-drawer.png'))
  await click(target, '.d2-brand')
  await waitForMissing(target, '.d2-drawer-layer.open')
  await waitForSelector(target, '.field-listening')
